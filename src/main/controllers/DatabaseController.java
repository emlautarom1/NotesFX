...
public class DatabaseController {
    ...
    private static boolean databaseHasTables() {
        boolean tableUsersExist = false;
        boolean tableNotesExist = false;
        try {
            ResultSet rs = getConnection().getMetaData().getTables(null, null, USERS_TABLE_NAME, null);
            while (rs.next()) {
    ...
    private static void createTables() throws SQLException {
        ...
        Statement usersTableStatement = getConnection().createStatement();
        ...
        Statement notesTableStatement = getConnection().createStatement();
    ...
    public static void insertNote(Note note) throws SQLException {
        ...
        Date convertedDate = new Date(note.getWriteDate().getTime());
    ...
    public static void updateNote(Note note) throws SQLException {
        PreparedStatement statement = getConnection().
                prepareStatement("UPDATE NOTES SET NOTE = ? WHERE ID = ?");
        statement.setString(1, note.getNote());
    ...
    public static List<Note> getNotesFromUser(String username) throws SQLException {
        PreparedStatement statement = getConnection().
                prepareStatement("SELECT * FROM NOTES WHERE USERNAME = ?");
        statement.setString(1, username);
        ResultSet foundNotes = statement.executeQuery();

        List<Note> noteList = new ArrayList<>();
        while (foundNotes.next()) {
            int resultID = foundNotes.getInt("ID");
            String resultNote = foundNotes.getString("NOTE");
            String resultUsername = foundNotes.getString("USERNAME");
    ...
    public static User getUser(String username) throws SQLException {
        ...
        statement.setString(1, username);
        ResultSet foundUsers = statement.executeQuery();
        // From the first, and only expected result, build a User instance.
        String resultUsername = foundUsers.getString("USERNAME");
        String resultName = foundUsers.getString("NAME");
        String resultPassword = foundUsers.getString("PASSWORD");
